...
export class AmbienteEmpComponent implements OnInit, OnDestroy {
  ...
  private socketSubscriptionSet = false; // Variable de control
    ...
    this.authService.getCurrentUser().subscribe(
      data => {
        ...
        if (this.user_employee && this.user_employee.helmet) {
          ...
          // Ejecuta getSensorData solo si no hay una suscripción de socket establecida
          if (!this.socketSubscriptionSet) {
            this.getSensorData();
          }
        }
      },
  ...
  private setupSocketSubscription() {
    ...
    this.socketSubscriptionSet = true; // Marca que la suscripción al socket está configurada
  }
  ...
  private updateHumidityStatus() {
    if (this.humidity > 70) {
      ...
      if (this.humidity == null) {
        this.humidityStatus = 'No se han detectado cambios en la humedad.';
      } else {
